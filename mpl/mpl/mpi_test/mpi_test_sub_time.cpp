...
using namespace std;
MPL_BEGIN
void mpi_test::test_sub_time(int n_sample, int opnd1_length, int opnd2_length, unsigned int seed)
{
	cout << "--------------------[减法]耗时测试--------------------" << endl;
	...
	MILLI_TIME(
		...
		{
			mpi::sub(opnd1[i], opnd2[i], r);
		}
	, 减法总耗时);
	cout << "平均每次减法耗时：" << double(time_count) / n_sample << "ms" << endl;
}
MPL_END